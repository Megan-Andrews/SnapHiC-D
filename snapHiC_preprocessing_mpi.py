...
def main():
    ...
    task_indices = list(range(mpi_rank, len(file_paths), mpi_size))
    task_filepaths = file_paths[task_indices]

    if (mpi_rank == 0) & (not os.path.exists(args.log_dir)):
        os.mkdir(args.log_dir)
    log_path = os.path.join(args.log_dir, 'rank{}_log.txt'.format(mpi_rank))
    log = open(log_path, 'w')
    ...
    for filepath in task_filepaths:
        process_cell(filepath, args.format, args.extension, args.output_dir, args.resolution, chromosome_lengths, args.chrom_columns, args.pos_columns, window_size, step_size, args.upper_distance, args.rp, log)
    mpi_comm.barrier()
...
def create_parser():
    ...
    parser.add_argument('--log-dir', action = 'store', required = True, help = 'log directory path')
